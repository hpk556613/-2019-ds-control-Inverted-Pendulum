...
//				Para1=1000;
				...
				Encoder_Bias=myabs(Encoder_Bias);
				Motor_PostionPID.SetPoint=Encoder_Balance;
				Encoder_Enter=Encoder_Bias;
				Xianfu_Pwm();                        									 //===电机编码器限幅 (每个圆周循环增加2pai)	
//				PID_SetPoint(&Motor_PostionPID,Para1);
				Para2=Inc_PID_Calc2(&Motor_PostionPID,Encoder_Enter);  /* 计数得到位置式PID的增量数值 */	
				Moto=Para2;
				Xianfu_Pwm();                         									//===PWM限幅 
//				if(Turn_Off()==0)
